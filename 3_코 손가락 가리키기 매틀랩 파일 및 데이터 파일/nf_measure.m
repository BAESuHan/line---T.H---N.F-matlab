...
for idx=1:10
    eval(['good' num2str(idx,'%02d') '= ''C:\Users\qotng\Desktop\GPG\매틀랩\기어_nf_0331\good\good(' num2str(idx,'%02d') ').json'';']);
...
for idx=1:10
    eval(['poor' num2str(idx,'%02d') '= ''C:\Users\qotng\Desktop\GPG\매틀랩\기어_nf_0331\poor\poor(' num2str(idx,'%02d') ').json'';']);
...
%% findpeak function
for idx=1:10
    eval(['[good_hp' num2str(idx,'%02d') ',good_hp_idx' num2str(idx,'%02d') ']=findpeaks(good_smo' num2str(idx,'%02d') ',''MinPeakDistance'',30);']);
    
    eval(['[poor_hp' num2str(idx,'%02d') ',poor_hp_idx' num2str(idx,'%02d') ']=findpeaks(poor_smo' num2str(idx,'%02d') ',''MinPeakDistance'',30);']);
end
...
%good
subplot(5,2,1);
%  plot(good_time04,good_sum04);
plot(good_time04,good_n_sum04,'Color',good_co);
ylim([-2 2])  
xlim([0 14])  
xticks([]);

subplot(5,2,5);
plot(good_time04,good_band_n04,'Color',good_co);
ylim([-1 1])
xlim([0 14]) 
yticks([]);

subplot(5,2,3);
plot(good_time04,good_smo04,'Color',good_co);
hold on
plot(good_time04(good_hp_idx04),good_hp04,'ok');
ylim([-1.5 1.8])
xlim([0 14]) 
yticks([]);

[pk,MaxFreq] = findpeaks(good_fft_band04,'NPeaks',1,'SortStr','descend');

subplot(5,2,9);
plot(good_freq04,good_fft_band04,'Color',good_co);
hold on
plot(good_freq04(MaxFreq),pk,'ok');
hold off
ylim([0 0.07])
%   xlabel('f(Hz)')
...
%%%poor
subplot(5,2,2);
%  plot(poor_time04,poor_sum04);
plot(poor_time04,poor_n_sum04,'Color',poor_co);
ylim([-2 2])
xlim([0 23]) 
xticks([]);

subplot(5,2,6);
plot(poor_time04,poor_band_n04,'Color',poor_co);
ylim([-1 1])
xlim([0 23])
yticks([]);

subplot(5,2,4);
plot(poor_time04,poor_smo04,'Color',poor_co);
hold on
plot(poor_time04(poor_hp_idx04),poor_hp04,'ok');
ylim([-1.5 1.8])
xlim([0 23])
yticks([]);

[pk,MaxFreq] = findpeaks(poor_fft_band04,'NPeaks',1,'SortStr','descend');
subplot(5,2,10);
plot(poor_freq04,poor_fft_band04,'Color',poor_co);
hold on
plot(poor_freq04(MaxFreq),pk,'ok');
hold off
ylim([0 0.07])
%   xlabel('f(Hz)')
...
function cnt = findPeakcount(smooth_data)
    [HP,HP_p]=findpeaks(smooth_data,'MinPeakDistance',30);
    idx=1;
    found_high=0;
    f_n_cnt=0;
    for i=1:length(smooth_data)
        if((HP_p(1,idx)==i))
            if(smooth_data(1,i)<=0.2)
                idx=idx+1; 
            else
                found_high=1;
                idx=idx+1;
